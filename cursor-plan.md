...
Also confirmed by sampling the pixels: every sheet is 32-bit ARGB, and the ink is a **single fully opaque color, `#141013`, with no anti-aliasing at all**. A one-color opaque shape is a usable CSS mask, which is what makes the recoloring approach below work. Within a 64x64 cell the figure occupies roughly x 9..48, y 11..49.
...
Nothing else needs to be acquired. One gap worth naming:

- `index.html` currently contains **no links, no buttons, and no form fields**, so there is nothing for Enter to click and no way to test that WASD and Space stay out of the way while typing. The plan adds one `<a>`, one `<button>`, and one `<input type="text">` so both paths are actually testable.
- The page is also **shorter than one viewport**, a heading and two paragraphs, so it does not scroll at all. Neither the Space suppression nor the edge auto-scroll can be observed on it as it stands. The plan adds enough filler content to make the page a few screens tall, with the test link placed below the fold so reaching it actually requires scrolling.
...
- A `@media (prefers-reduced-motion: reduce)` block, paired with the JS bail-out below.

**New — `work/p1-website/cursor.js`** (loaded `defer`, creates its own DOM, so a no-JS visitor sees nothing unusual)

1. **Bail-outs first.** Do nothing unless `matchMedia('(pointer: fine)').matches` — no custom cursor on touch devices. Do nothing when `prefers-reduced-motion: reduce`. Only after passing both, create the element and add `.stickman-active`.
...
9. **Hotspot at the cell center (32, 32)**, so the sprite is drawn at `translate(x - 32, y - 32 + jumpOffset)`. Center is chosen because `scaleX(-1)` mirrors about it, meaning the hotspot does not jump when the figure turns around. Keep it a named constant, it is worth tuning by eye.
...
**Edited — `work/p1-website/index.html`**

- `<link rel="stylesheet" href="cursor.css">` in `<head>`, after the existing `style.css`.
- `<script src="cursor.js" defer></script>` before `</body>`.
- One `<a href="...">`, one `<button>`, and one `<input type="text">` in the body, so Enter has something real to hit and the typing and focus guards have something to be tested against.
...
4. **Punch** — press Enter with the stickman over the test link. The 10-frame punch plays once, does not loop, and the page navigates. Press Enter over empty space: punch plays, nothing navigates.
...
14. **Other scrolling still works** — mouse wheel, arrow keys, and PageDown all scroll normally. Only Space was taken.
15. **Dark mode** — toggle Windows dark mode. The stickman is clearly visible against both backgrounds.
16. **Touch and reduced motion** — DevTools device toolbar, and DevTools rendering panel's "emulate prefers-reduced-motion". In both, the native arrow comes back and no sprite appears.
17. **Network tab** — `Thin.png`, `Run.png`, `Punch.png`, `Jump.png` and `JumpDown.png` all return 200, `JumpUp.png` is never requested, and there are no red 404s. Path bugs are the failure mode the P1 README warns about hardest, and this is what catches them.
18. **Toggle** — press Escape. Native arrow returns, sprite disappears, any in-flight scroll stops, page still works.

The checks most likely to actually fail are 17 (paths), then 6 (`preventDefault` on the wrong event, so Space still scrolls), then 9 (auto-scroll leaking into follow mode), then 12.
